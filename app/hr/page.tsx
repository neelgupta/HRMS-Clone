...
export default function HRDashboardPage() {
  ...
  if (!user) {
    return <div className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-700 dark:border-rose-700 dark:bg-rose-900/20 dark:text-rose-400">{error || "Unauthorized"}</div>;
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-lg shadow-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:shadow-slate-900/10">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-indigo-600 dark:text-indigo-400">Overview</p>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-700/50">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">{companyName}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-700/50">
              <p className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">{user.role.replace("_", " ")}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-700/50">
              <p className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400">
              ...
              <p className="mt-2 truncate text-xl font-semibold text-slate-950 dark:text-white">{user.companyId}</p>
            </div>
          </div>
        </section>

        <aside className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-lg shadow-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:shadow-slate-900/10">
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-indigo-600 dark:text-indigo-400">Action Center</p>
