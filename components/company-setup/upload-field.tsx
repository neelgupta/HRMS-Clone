...
export function UploadField({ label, value, accept = "image/*", onUploaded }: UploadFieldProps) {
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-700/50">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{label}</p>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{value ? "Uploaded and ready." : "Upload PNG, JPG, or SVG."}</p>
        </div>
        <button
          type="button"
        ...
        >
          {uploading ? <Spinner className="text-current dark:text-white" label="Uploading asset" /> : null}
          {uploading ? "Uploading..." : value ? "Replace" : "Upload"}
        </button>
      </div>

      {value ? (
        <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 dark:border-slate-600 dark:bg-slate-800">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={value} alt={label} className="h-24 w-full rounded-xl object-contain bg-slate-50 dark:bg-slate-700" />
        </div>
      ) : null}
